...
public class SinglePageSelectorMenu<T> extends SelectorMenu<T> {
    public SinglePageSelectorMenu(ServerPlayer player, Collection<T> options, Consumer<Selection<T>> onSelect, LabelMap<T> labelMap) {
        super(getSmallestFitting(options.size()).getFirst(), player, options, onSelect, labelMap);

        for (int i = 0; i < this.options.size(); i++) {
            T option = this.options.get(i);
            this.setSlot(i, GuiElementBuilder.from(labelMap.getLabel(option)).setCallback(() -> this.finish(new Selection<>(true, option))));
        }
