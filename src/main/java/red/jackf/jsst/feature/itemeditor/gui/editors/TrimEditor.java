...
public class TrimEditor extends GuiEditor {
    ...
    private void drawStatic() {
        ...
        for (int row = 0; row < 4; row++) {
            this.setSlot(4 + Util.slot(3, row), CommonLabels.divider());
            this.setSlot(4 + Util.slot(7, row), CommonLabels.divider());
        }

        this.setSlot(4 + Util.slot(8, 3), CommonLabels.cancel(this::cancel));
    }
    ...
    protected void redraw() {
        ...
        final int addOrRemoveSlot = 4 + Util.slot(8, 0);
        if (this.trim == null) {
            ...
            ItemStack materialPreview = this.trim.material().value().ingredient().value().getDefaultInstance();
            this.setSlot(Slots.MATERIAL, GuiElementBuilder.from(materialPreview)
                    .setName(this.trim.material().value().description())
                    .hideFlags());

            ItemStack patternPreview = this.trim.pattern().value().templateItem().value().getDefaultInstance();
            this.setSlot(Slots.PATTERN, GuiElementBuilder.from(patternPreview)
                    .setName(this.trim.pattern().value().description())
                    .hideFlags());
        }
    }
}
